<?php
class ProfileModel extends Model implements IModel
{
    private $id;
    private $title;
    private $content;
    ...
    public function getProfileLimit($n, $toArray = false)
    {
        $items = [];
        try {
    ...
    public function getProfilesByUserId($userId, $n = 10, $toArray = false)
    {
        $items = [];
        try {
            $pdo = $this->db->connect(); // Obtener la instancia de PDO
            $query = $pdo->prepare('SELECT * FROM hilo WHERE usuario_id = :userId ORDER BY fecha_creacion DESC LIMIT :n
            ');
            $query->execute(['userId' => $userId, 'n' => $n]);
            if ($query->rowCount() > 0) {
                while ($p = $query->fetch(PDO::FETCH_ASSOC)) {
                    $item = new ProfileModel();
                    $item->from($p);
    ...
    public function update()
    {
        try {
            $pdo = $this->db->connect(); // Obtener la instancia de PDO
            $query = $pdo->prepare('UPDATE FROM hilo SET titulo = :titulo, contenido = :contenido, fecha_creacion = :fecha_creacion, usuario_id = :usuario_id  WHERE id = :id');
            $query->execute([
                'titulo' => $this->title,
                'contenido' => $this->content,
                'usuario_id' => $this->userId,
